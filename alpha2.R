...
setwd("~/Desktop/ANSC516/ANSC516-repo/data/moving-pictures")
list.files()
getwd()
...
meta<-read_q2metadata("Assignment7/sample-metadata.tsv")
str(meta)
...
evenness = read_qza("Assignment7/evenness_vector.qza")
...
observed_features = read_qza("Assignment7/observed_features_vector.qza")
...
shannon = read_qza("Assignment7/shannon_vector.qza")
...
faith_pd = read_qza("Assignment7/faith_pd_vector.qza")
...
meta2<-read_q2metadata("Assignment7/sample-metadata.tsv")
...
meta5 = merge(meta2, alpha_diversity, by.x = "SampleID", by.y = "SampleID")
row.names(meta) <- meta$SampleID
row.names(alpha_diversity) <- alpha_diversity$SampleID
#meta = meta[,-1]
str(meta2)
meta2$SampleID <- as.character(meta2$SampleID)
meta2$SampleID <- trimws(meta2$SampleID)
str(alpha_diversity)
...
#Plots
hist(alpha_diversity$shannon_entropy, main="Shannon diversity", xlab="", breaks=10)
hist(alpha_diversity$faith_pd, main="Faith phylogenetic diversity", xlab="", breaks=10)
hist(alpha_diversity$pielou_e, main="Evenness", xlab="", breaks=10)
hist(as.numeric(alpha_diversity$observed_features), main="Observed Features", xlab="", breaks=10)
...
aov.observed_features.treatment = aov(observed_features ~ treatment, data=meta5)
...
levels(meta5$treatment)
#Re-order the groups because the default is alphabetical order
meta5$treatment.ord = factor(meta5$treatment, c("NC", "Fiber"))
levels(meta5$treatment.ord)

#Plot
boxplot(pielou_evenness ~ treatment.ord, data=meta5, ylab="Peilou evenness")

evenness_boxplot <- ggplot(meta5, aes(treatment.ord, pielou_evenness)) + 
  geom_boxplot() + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("Assignment7/output/evenness_boxplot.png", evenness_boxplot, height = 3, width = 3)
...
evenness_summary <- meta5 %>% # the names of the new data frame and the data frame to be summarised
...
ggsave("Assignment7/output/evenness_se.png", evenness_se, height = 2.5, width = 3)
...
kruskal.test(faith_pd ~ treatment.ord, data=meta5)
...
pairwise.wilcox.test(meta5$faith_pd, meta5$treatment.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(faith_pd ~ treatment.ord, data=meta5, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd_boxplot <- ggplot(meta5, aes(treatment.ord, faith_pd)) + 
...
ggsave("Assignment7/output/pd.png", faith_pd_boxplot, height = 3, width = 3)


# Wilcoxon Rank Sum Test for two variables
str(meta5)
levels(meta5$treatment)
meta5$treatment <- factor(trimws(meta5$treatment))
meta5$treatment.ord <- factor(meta5$treatment.ord, 
                              levels = c("Fiber", "NC"), 
                              labels = c("Fiber", "NC"))
wilcox.test(faith_pd ~ treatment, data = meta5)

# Plotting the results with ggplot2
faith_pd_boxplot <- ggplot(meta5, aes(treatment, faith_pd)) + 
...
# Save the plot
ggsave("Assignment7/output/pd_wilcoxon.png", faith_pd_boxplot, height = 3, width = 3)
...
glm.evenness.time = glm(pielou_evenness ~ treatment, data=meta5)
...
plot(pielou_evenness ~ treatment, data=meta5)
#Add the glm best fit line
plot(pielou_evenness ~ treatment, data=meta5) + abline(glm.evenness.time)
...
gaussian.faith.time = glm(faith_pd ~ treatment, data=meta5, family="gaussian")
plot(gaussian.faith.time, which=c(1,2))

# Quasipoisson (log) distribution
qp.faith.time = glm(faith_pd ~ treatment, data=meta5, family="quasipoisson")
...
#Plot
plot(log(faith_pd) ~ treatment, data=meta5, ylab="ln(Faith Phylo. Diversity)")
plot(log(faith_pd) ~ treatment, data=meta5, ylab="ln(Faith Phylo. Diversity)") + abline(qp.faith.time)
